...
extern "C" void app_main(void) {
  ...
  if (err != ESP_OK) {
    ESP_LOGE(TAG,
             "No se pudo inicializar el BME690 (err=%s). Verificar cableado.",
             esp_err_to_name(err));
  ...
  while (true) {
    ...
    if (err == ESP_OK) {
      ...
      if (sensor_data.gas_valid && sensor_data.heater_stable) {
        ESP_LOGI(TAG, "Gas (VOC)    : %lu Ohm",
                 (unsigned long)sensor_data.gas_resistance);
      } else {
        ESP_LOGW(TAG,
                 "Gas (VOC)    : medición no válida "
