...
esp_err_t manejo_init(const manejo_config_t *config, manejo_handle_t *handle) {
  ...
  // Configurar nSLEEP si está asignado (Activo en ALTO = Wakeup)
  if (config->nSLEEP_M1 >= 0) {
    gpio_config_t cfg_sleep = {
        .pin_bit_mask = (1ULL << config->nSLEEP_M1),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    ESP_ERROR_CHECK(gpio_config(&cfg_sleep));
    gpio_set_level((gpio_num_t)config->nSLEEP_M1, 1);
  }
  if (config->nSLEEP_M2 >= 0) {
    gpio_config_t cfg_sleep = {
        .pin_bit_mask = (1ULL << config->nSLEEP_M2),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    ...
    ESP_ERROR_CHECK(gpio_config(&cfg_sleep));
    gpio_set_level((gpio_num_t)config->nSLEEP_M2, 1);
  }

  // Configurar DISABLE si está asignado (Activo en BAJO = Enable H-Bridge)
  if (config->DISABLE_M1 >= 0) {
    gpio_config_t cfg_dis = {
        .pin_bit_mask = (1ULL << config->DISABLE_M1),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    ESP_ERROR_CHECK(gpio_config(&cfg_dis));
    gpio_set_level((gpio_num_t)config->DISABLE_M1, 0);
  }
  if (config->DISABLE_M2 >= 0) {
    gpio_config_t cfg_dis = {
        .pin_bit_mask = (1ULL << config->DISABLE_M2),
        .mode = GPIO_MODE_OUTPUT,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    ...
    ESP_ERROR_CHECK(gpio_config(&cfg_dis));
    gpio_set_level((gpio_num_t)config->DISABLE_M2, 0);
  }

  // Esperar tiempo t(WAKE) de 1.5ms para despertar el integrado
